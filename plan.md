...
Convención del template de `create-expo-app`: todo el código de la app vive bajo `/src`, y dentro de `/src/app` solo van pantallas y layouts (ruteo por archivos de `expo-router`).
```
/src
...
**Regla:** todo mock vive bajo `/src/mocks`, expone funciones `async` que devuelven una `Promise` resuelta tras 500–1000 ms. El día que exista backend real, solo se reemplaza esa capa.
